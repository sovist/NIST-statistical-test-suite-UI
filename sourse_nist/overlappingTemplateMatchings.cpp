...
OverlappingTemplateMatchings(int m, int n)
{
	...
	lambda = (double)(M-m+1)/pow((double)2,m);
	eta = lambda/2.0;
	sum = 0.0;
	...
	fprintf(stats[TEST_OVERLAPPING], "\t\t-----------------------------------------------\n");
	fprintf(stats[TEST_OVERLAPPING], "\t\t%3d %3d %3d %3d %3d %3d  %f ", nu[0], nu[1], nu[2], nu[3], nu[4], nu[5], chi2);
...
Pr(int u, double eta)
{
	...
	if ( u == 0 )
		...
		sum = 0.0;
		for ( l=1; l <= u; l++ )
			sum += exp(-eta-u*log((double)2)+l*log(eta)-cephes_lgam(l+1)+cephes_lgam(u)-cephes_lgam(l)-cephes_lgam(u-l+1));
		p = sum;
	}
	return p;
}
