...
ApproximateEntropy(int m, int n)
{
	...
	for ( blockSize=m; blockSize<=m+1; blockSize++ ) {
		if ( blockSize == 0 ) {
			...
			powLen = (int)pow((double)2, blockSize+1)-1;
			...
			/* DISPLAY FREQUENCY */
			sum = 0.0;
			index = (int)pow((double)2, blockSize)-1;
			for ( i=0; i<(int)pow((double)2, blockSize); i++ ) {
				if ( P[index] > 0 )
	...
	chi_squared = 2.0*seqLength*(log((double)2) - apen);
	p_value = cephes_igamc(pow((double)2, m-1), chi_squared/2.0);
	...
	if ( m > (int)(log((double)seqLength)/log((double)2)-5) ) {
		fprintf(stats[TEST_APEN], "\t\tNote: The blockSize = %d exceeds recommended value of %d\n", m,
			MAX(1, (int)(log((double)seqLength)/log((double)2)-5)));
