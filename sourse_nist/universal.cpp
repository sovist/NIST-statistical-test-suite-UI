...
void Universal(int n)
{
	...
	Q = 10*(int)pow((double)2, L);
	K = (int) (floor((double)n/L) - (double)Q);	 		    /* BLOCKS TO TEST */
	
	p = (int)pow((double)2, L);
	if ( (L < 6) || (L > 16) || ((double)Q < 10*pow((double)2, L)) ||
	...
	for ( i=1; i<=Q; i++ ) {		/* INITIALIZE TABLE */
		decRep = 0;
		for ( j=0; j<L; j++ )
			decRep += epsilon[(i-1)*L+j] * (long)pow((double)2, L-1-j);
		T[decRep] = i;
	}
	for ( i=Q+1; i<=Q+K; i++ ) { 	/* PROCESS BLOCKS */
		...
		for ( j=0; j<L; j++ )
			decRep += epsilon[(i-1)*L+j] * (long)pow((double)2, L-1-j);
		sum += log((double)i - T[decRep])/log((double)2);
		T[decRep] = i;
	}
