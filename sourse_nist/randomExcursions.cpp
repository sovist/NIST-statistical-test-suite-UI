...
RandomExcursions(int n)
{
	...
	if ( ((S_k = (int *)calloc(n, sizeof(int))) == NULL) ||
		 ((cycle = (int *)calloc( MAX(1200, n/20), sizeof(int))) == NULL) ) 
    {
		...
		if ( cycle != NULL )
			free(cycle);
        throw gcnew Exception("Random Excursions Test:\n" +  
                              "Insufficent Work Space Allocated.");
		//return;
	}
	...
	for( i=1; i < n; i++ ) 
    {
		...
		if ( S_k[i] == 0 ) 
        {
			...
			if ( J > MAX(1200, n/20) ) 
            {
				...
				free(cycle);
				//return;
                throw gcnew Exception("ERROR IN FUNCTION randomExcursions:\n" +  
