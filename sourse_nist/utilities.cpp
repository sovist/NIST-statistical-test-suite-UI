		...
		switch( option ) 
		{
			case 0:
				...
				*streamFile = (char*)calloc(200, sizeof(char));
                sprintf(*streamFile, "%s", /*file*/ fileName);
				//printf("\n");
...
//	int		mode;
	...
	printf("    [0] ASCII - Each byte in data file contains 8 bits of data\n\n"); 
	printf("    [1] Binary - A sequence of ASCII 0's and 1's\n");
	...
	for ( i=0; i<tp.numOfBitStreams; i++ ) 
    {
		...
		for ( j=0; j<tp.n; j++ ) 
        {
			if ( fscanf(fp, "%1d", &bit) == EOF ) 
            {
				...
				fclose(fp);
				free(epsilon);
                throw gcnew Exception("ERROR:  Insufficient data in file. " + bitsRead +" bits were read.");
			}
			else
            {
				bitsRead++;
		...
		do 
        {
			if ( fread(buffer, sizeof(unsigned char), 4, fp) != 4 ) 
            {
                ...
                throw gcnew Exception("READ ERROR:  Insufficient data in file.\n"
					"File size is < " + ((double)tp.n*tp.numOfBitStreams) + " bit " + 
					"(" + (((((double)tp.n*tp.numOfBitStreams)/8)/1024)/1024).ToString("f2") + "Mb)");
				//return;
			}
...
openOutputStreams(int option)
{
    ...
    {
        throw gcnew Exception("MAIN: Could not open freq file: " + (const char)freqfn);
    ...
    {
        throw gcnew Exception("MAIN: Could not open stats file: " + (const char)summaryfn);
		...
		if ( testVector[i] == 1 ) 
        {
            ...
            {	/* STATISTICS LOG */
                ...
                throw gcnew Exception("ERROR: LOG FILES COULD NOT BE OPENED.\n" + 
                                      "MAX # OF OPENED FILES HAS BEEN REACHED = " + numOfOpenFiles + 
...
void
invokeTestSuite(int option, char *streamFile, int modeReadFile)
{
	...
	switch( option ) {
		case 0:
			fileBasedBitStreams(streamFile, modeReadFile);
			break;
		case 1:
			lcg();
			break;
...
void nist_test_suite()
{
	...
	for(int i = 0; i < 15; i++)//чекати завершення всіх потоків;
		tr[i]->Join();

    _initParam->incProgress();
