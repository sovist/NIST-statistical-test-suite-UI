...
Serial(int m, int n)
{
	...
	p_value1 = cephes_igamc(pow((double)2, m-1)/2, del1/2.0);
	p_value2 = cephes_igamc(pow((double)2, m-2)/2, del2/2.0);
...
psi2(int m, int n)
{
	...
	sum = 0.0;
	for ( i=(int)pow((double)2, m)-1; i < (int)pow((double)2, m+1)-1; i++ )
		sum += pow((double)P[i], 2);

	sum = (sum * pow((double)2, m)/(double)n) - (double)n;
